    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
                ...
                <form action="{{ route('marca.index') }}" method="GET" class="flex gap-2 w-full md:w-auto">
                    <div class="relative flex-grow">
                        <i class="fas fa-search absolute left-3 top-3 text-gray-400"></i>
                        <input type="text" name="search" placeholder="Buscar marca..."
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach ($marcas as $mrc)
                                <tr>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $mrc->id }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $mrc->nome }}</td>
                                    <td class="px-6 py-4 text-sm text-gray-500">{{ $mrc->descricao ?? '-' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <a href="{{ route('marca.edit', $mrc->id) }}"
                                               class="inline-flex items-center px-2 py-1 bg-yellow-500 hover:bg-yellow-600 text-white text-xs rounded transition"
                                               title="Editar">
                                                <i class="fas fa-edit mr-1"></i> Editar
                                            </a>

                                            <form action="{{ route('marca.destroy', $mrc->id) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
            ...
            @else
                <div class="bg-gray-50 p-8 rounded-md text-center">
                    ...
                    <p class="text-gray-600 text-lg">Nenhuma marca cadastrada</p>
                    @if(request('search'))
                        <p class="text-sm text-gray-500 mt-2">
                            Nenhum resultado encontrado para "{{ request('search') }}".
                            <a href="{{ route('marca.index') }}" class="text-blue-600 hover:underline">Limpar busca</a>
                        </p>
                    @else
                        <a href="{{ route('marca.create') }}"
                           class="mt-4 inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-white hover:bg-green-700 transition">
                            <i class="fas fa-plus mr-2"></i> Cadastrar primeira marca
                        </a>
                    @endif
                </div>
            @endif
